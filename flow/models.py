...
# from modules import Flow, ActNorm, AdditiveCoupling, InvConv2dLU, ActNormIdentity, Permute2d
# from ops import compute_z_shapes, sample_z
...
class FLOW(object):
    ...
    def init_module(self):
        ...
        #Assign devices
        device0 = torch.device("cuda:0") if torch.cuda.device_count() > 0 else "cpu"  # Generator (GPU 0,1)
        device1 = torch.device("cuda:2") if torch.cuda.device_count() > 2 else device0  # Discriminator (GPU 2,3)
        ...
        generator = Flow(
            ...
            n_bits=5,
            **network_configs[opt.network_config]
        ).to(device0)  # Move generator to GPU 0
        ...
        y_classifier = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Linear(512, dim_condition),
        ).to(device1)  # Move classifier to GPU 1
        ...
        self.y_classifier = y_classifier
        self.optimizer = optimizer
        self.device0 = device0  # Store GPU 0
        self.device1 = device1  # Store GPU 1

    # def init_module(self, device_ids=[0, 1]):
    #     opt = self.opt
    #     network_configs = [
    #         {'norm_layer': ActNorm, 'coupling_layer': AdditiveCoupling, 'permute_layer': InvConv2dLU},
    #         {'norm_layer': ActNormIdentity, 'coupling_layer': AdditiveCoupling,
    ...
    #     # 🔥 Assign GPUs based on the process
    #     if self.opt.local_rank < 2:
    #         device0 = torch.device("cuda:0")  # GPU 0 for generator
    #         device1 = torch.device("cuda:1")  # GPU 1 for classifier
    #     else:
    #         device0 = torch.device("cuda:2")  # GPU 2 for generator
    #         device1 = torch.device("cuda:3")  # GPU 3 for classifier
    ...
    #         n_bits=5,
    #         **network_configs[opt.network_config]
    #     ).to(device0)  # ✅ Move generator to GPU 0 or 2
    ...
    #         nn.ReLU(inplace=True),
    #         nn.Linear(512, dim_condition),
    #     ).to(device1)  # ✅ Move classifier to GPU 1 or 3
    ...
    #     self.y_classifier = y_classifier
    #     self.optimizer = optimizer
    #     self.device0 = device0  # Store first GPU
    #     self.device1 = device1  # Store second GPU
    ...
    def train(self, inputs, n_iter):
        opt = self.opt
        image, labels = inputs[0].to(self.device0), inputs[1].to(self.device1)  # Move input to different GPUs

        log_p, log_det, z_list = self.generator(input=image, condition=None, continuous=True)

        nll_loss, log_p, log_det = self.calc_loss(log_p.to(self.device1), log_det.to(self.device1))
        y_logits = self.y_classifier(torch.flatten(z_list[-1].to(self.device1), start_dim=1))  # Move latent space output to GPU 1
        ...
        loss = nll_loss.to(self.device1) + cls_loss.to(self.device1) * opt.cls_loss_weight
    ...
    def post_task(self, n_iter):
        ...
        if n_iter % opt.generation_iter == 0:
            z_samples = sample_z(input_shape=(opt.n_sample, 3, opt.img_size, opt.img_size),
                                 n_stages=opt.n_block, temp=opt.temp)
            z_samples = [x.cuda(0) for x in z_samples]  # ✅ Generate images on GPU 0
            gen_images = self.generator.reverse(z_list=z_samples, condition=None)
            grid_img = torchvision.utils.make_grid(gen_images, normalize=True)
            self.logger.save_image(grid_img, n_iter, 'train')
